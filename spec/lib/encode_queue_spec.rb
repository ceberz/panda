...
require 'encode_queue.rb'

describe EncodeQueue, "instantiation" do
  before(:each) do    
  ...
  it "should create a queue named appropriately from the namespace if such a queue does not exist" do
    ...
    mocked_sqs.should_receive(:create_queue).once.with("humanized_phrase_encode_queue", anything) 
  ...
  it "should discover the URI for an existing queue" do
    mocked_sqs = stub_everything("mocked sqs")
    mocked_sqs.should_receive(:queue_url_by_name).once.with('humanized_phrase_encode_queue').and_return("queue URI")
    RightAws::SqsGen2Interface.stub!(:new).and_return(mocked_sqs)
