...
describe NotifierSingleton, "job processing" do
  before(:each) do
    ...
    Video.stub!(:delete_notification_job)
  end
  ...
  it "should pull notification tickets from the queue and process each in turn" do
    ...
    Video.should_receive(:delete_notification_job).once.ordered.with(@job_hash_1[:receipt])
    @mocked_video_2.should_receive(:send_notification).once
    Video.should_receive(:delete_notification_job).once.ordered.with(@job_hash_2[:receipt])
    
    NotifierSingleton.process_notifications
  end
  
  it "should remove succesfull notifications from the queue" do
    @mocked_job_queue.stub!(:dequeue).and_return([@job_hash_1, @job_hash_2])
    @mocked_video_1.stub!(:send_notification)
    @mocked_video_2.stub!(:send_notification)
  ...
  it "should remove unsuccesfull notifications from the queue" do
    @mocked_job_queue.stub!(:dequeue).and_return([@job_hash_1, @job_hash_2])
    @mocked_video_1.stub!(:send_notification).and_raise(Exception)
    @mocked_video_2.stub!(:send_notification).and_raise(Exception)
